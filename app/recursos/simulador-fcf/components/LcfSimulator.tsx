...
export default function LcfSimulator() {
  ...
  // Calculate User BPM from taps - MÉTODO CORRECTO
  const calculateUserBPM = useCallback(() => {
    ...
    // MÉTODO CORRECTO: Calcular BPM basado en tiempo total y número de latidos
    // Fórmula: BPM = (número de latidos × 60) / tiempo transcurrido en segundos
    
    // Calcular el tiempo total en segundos desde el primer hasta el último tap
    const tiempoTotal = (taps[taps.length - 1].timestamp - taps[0].timestamp) / 1000; // ms a segundos
    
    // Número de latidos = número de taps
    const numeroLatidos = taps.length;
    
    // Calcular BPM
    const calculatedBPM = Math.round((numeroLatidos * 60) / tiempoTotal);
  ...
  return (
    <div className="bg-gradient-to-br from-red-50 to-rose-100 rounded-xl shadow-lg p-4 border-2" style={{ borderColor: '#dc2626' }}>
      {/* Header Compacto */}
      <div className="flex items-center gap-2 mb-3">
        <div className="p-2 rounded-lg" style={{ background: '#dc2626' }}>
      ...
      <div className="grid lg:grid-cols-2 gap-3 mb-3">
        ...
        <div className="bg-slate-900 rounded-lg p-2 shadow border border-red-900 relative">
          {beatIndicator && (
            <div className="absolute top-1 right-1 flex items-center gap-1 px-2 py-0.5 rounded-full bg-red-500 text-white text-xs font-bold animate-pulse">
              <span className="w-1.5 h-1.5 rounded-full bg-white"></span>
              Lub-Dub
            </div>
          )}
          <div className="mb-1 flex items-center justify-between text-[10px]">
            <span className="font-mono text-red-400">📊 Onda Real</span>
          ...
          <canvas
            ...
            width={500}
            height={80}
            className="w-full h-auto rounded border border-red-900"
          />
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="bg-white rounded-lg p-3 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <div>
                ...
                <div className="flex items-baseline gap-1">
                  <span className="text-2xl font-bold" style={{ color: '#dc2626' }}>{targetBPM}</span>
            ...
            <input
              ...
              style={{
                background: `linear-gradient(to right, #dc2626 0%, #dc2626 ${((targetBPM - 80) / 120) * 100}%, #e2e8f0 ${((targetBPM - 80) / 120) * 100}%, #e2e8f0 100%)`
              }}
            />
      ...
      <div className="grid lg:grid-cols-3 gap-3 mb-3">
        ...
        <div className="lg:col-span-1 bg-white rounded-lg p-2 shadow-sm">
          ...
          <div className="flex gap-1 mb-2">
            <button
              ...
              style={timerDuration === 6 ? { background: '#dc2626' } : {}}
            >
              6s
            </button>
            <button
              ...
              style={timerDuration === 10 ? { background: '#dc2626' } : {}}
            >
              10s
            </button>
          </div>
          <div className="text-xs font-semibold text-gray-700 mb-0.5">Tiempo</div>
          <div className="text-xl font-bold mb-1" style={{ color: '#dc2626' }}>
        ...
        <div className="lg:col-span-2">
          <button
            ...
            style={{
              background: isTimerRunning ? '#dc2626' : undefined,
              boxShadow: isTimerRunning ? '0 4px 20px rgba(220, 38, 38, 0.3)' : undefined
            }}
          >
            {isTimerRunning ? (
      ...
      {showResult && userBPM !== null && (
        <div className="bg-white rounded-lg p-3 border-2 shadow animate-fade-in" style={{ borderColor: '#dc2626' }}>
          ...
          <div className="bg-gradient-to-r from-red-500 to-rose-600 rounded-lg p-2 text-white mb-2 text-sm">
